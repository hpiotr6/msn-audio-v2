...
class WavDataset(BaseRawAudioDataset):
    ...
    def get_audio(self, index):
        ...
        ), f"Convert .wav files to {self.cfg.sample_rate} Hz. {filename} has {sr} Hz."
        return wav[0]
...
class WaveInLMSOutDataset(Dataset):
    ...
    def __init__(
        self, cfg, audio_files, labels, tfms, use_librosa=False, random_crop=False
    ):
        # argment check
